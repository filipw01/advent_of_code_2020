...
pub fn find_valid_combinations() -> usize {
    ...
    lines.pop();
    let mut adapters = lines.iter().map(|line| line.parse().unwrap()).collect::<Vec<usize>>();
    adapters.sort();
    ...
    adapters.iter().for_each(|adapter| {
        ...
        results.iter().for_each(|(result, _)| {
            if result + 1 == *adapter || result + 2 == *adapter || result + 3 == *adapter {
                let new_result = results_clone.get(adapter).unwrap_or(&0) + results_clone.get(result).unwrap_or(&0);
