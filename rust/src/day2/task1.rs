...
pub fn check_password() -> Result<usize, &'static str> {
    ...
    let valid_passwords: Vec<_> = lines.iter().map(|line| {
        ...
        let split_3: Vec<i32> = split_2.get(0).unwrap().split("-").map(|count| count.parse().unwrap()).collect();
        ...
        let letter_count = policy.password.chars().filter(|char| *char == policy.letter).count();
