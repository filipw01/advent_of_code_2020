...
fn find_containers<'a>(searched_bag: &'a str, bags: HashMap<&'a str, Vec<&str>>) -> HashSet<&'a str> {
    let mut containers = HashSet::new();
    for (color, content) in &bags {
        if content.contains(&searched_bag) {
            containers.insert(*color);
            containers = containers.union(&find_containers(color, bags.clone())).map(|x|*x).collect();
        }
    }
    containers
}
