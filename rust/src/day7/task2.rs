...
pub fn count_bags() -> usize {
    ...
    for line in lines {
        ...
        let inner_bags = content.split(", ").into_iter()
            .map(|bag| {
                let mut split = bag.trim_matches(|c: char| ['b', 'a', 'g', 's'].contains(&c))
                    .split(' ');
...
fn count_inner_bags(searched_bag: &str, bags: &HashMap<&str, Vec<(usize, String)>>, multiplier: Option<usize>) -> usize {
    let mut count = 0;

    for (inner_count, inner_color) in bags.get(searched_bag).unwrap_or(&vec![]) {
        count += (inner_count + count_inner_bags(inner_color.as_str(), bags, Some(*inner_count))) * multiplier.unwrap_or(1);
    }
    count
}
